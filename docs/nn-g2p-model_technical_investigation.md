...
- 改善: `HF_TOKEN` を使う取得手順を整備し、Private repo 前提の運用手順を docs に残す。  
...
## 7. GPU NaN 調査アップデート（2026-02-26）

- 詳細レポート: `docs/gpu_nan_operator_investigation_2026-02-26.md`
- 結論:
  - 現行モデル + 現行パッケージ（Unity 6000.3.6f1 / `com.unity.ai.inference` 2.4.1）では GPU backend が NaN を返し、実運用不可。
  - CPU backend では同一プローブで NaN なし。
- 実装反映:
  - `NnG2pSentisRuntime` に初期化時 GPU 数値健全性チェックを追加。
  - GPU 出力が NaN の場合は即 CPU へフォールバックして推論継続。

### ロードマップ修正（GPU関連）

1. 短期: CPU運用を既定にし、GPUは「検証用オプション」扱いで維持  
2. 中期: `com.unity.ai.inference` 更新版で再検証（同一プローブを再実行）  
3. 中期: 最小再現ケースを upstream に共有して修正可否を確認  
4. 長期: GPU必須要件がある場合は別ランタイム経路（Sentis外）を評価
